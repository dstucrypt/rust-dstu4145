...
mod test {
    ...
    #[test]
    fn test_field_neg_bytes() {
        let mod257 = gf2m::compute_modulus_bytes(257, 12, 0, 0);
        let value_a = gf2m::parse_hex(b"aff3ee09cb429284985849e20de5742e194aa631490f62ba88702505629a65890");
        let expect = gf2m::parse_hex(b"f5ae84d0c4dc2e7e89c670fb2083d124be50b413efb6863705bd63a5168352e0");

        assert_eq!(gf2m::neg_bytes(&value_a, &mod257), expect);
    }
    ...
    #[test]
    fn test_point_double_bytes() {
        let mod257 = gf2m::compute_modulus_bytes(257, 12, 0, 0);
        let curve_a = gf2m::zero();
        let point = curve2::Point {
        ...
        assert_eq!(
            curve2::point_dbl(&point, &mod257, &curve_a),
            curve2::Point {
    ...
    fn test_point_add_bytes() {
        ...
        let point_2 = curve2::Point {
            x: gf2m::parse_hex(b"176dbde19773dfd335665597e8d6a0ab678721a5bb7030f25dc4c48b809ef3520"),
            y: gf2m::parse_hex(b"6e75301556ea5d571403086691030f024c026907c8e818b2eedd9184d12040ee")
        };

        assert_eq!(
            curve2::point_add(&point, &point_2, &mod257, &curve_a),
            curve2::Point {
    ...
    fn test_point_mul_bytes() {
        ...
        assert_eq!(
            curve2::point_mul(&point, &privd, &mod257, &curve_a),
            curve2::Point {
    ...
    #[test]
    fn test_point_expand_bytes() {
        let curve = dstu_params::curve_431_bytes();
        let point_data: [u8; 54] = [
            0xb6, 0x1b, 0xf9, 0xbd, 0x4b, 0x62, 0xca, 0xab,
            0x2c, 0x39, 0x50, 0xf5, 0xc5, 0x1d, 0x5f, 0xa8,
            0x0d, 0x70, 0x7e, 0x00, 0x7b, 0x52, 0x5b, 0x70,
        ...
        let point = curve2::point_expand(&point_compressed_x, &curve);
        assert_eq!(point, curve2::Point {
